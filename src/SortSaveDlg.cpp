...
BOOL CSortSaveDlg::OnInitDialog() 
{
	...
	static char tmp[5]; 
	//load bmp names
	CString strTemp = ((CVeryIEApp*)AfxGetApp())->m_strDefaultDirSort;
	cur[0]=0;
...
void CSortSaveDlg::OnRename() 
{
	...
	if (dlg.DoModal()==IDOK)
	{
		...
		if (strName.GetLength())
		{
			...
			//		
			CString strOldPath = ((CVeryIEApp*)AfxGetApp())->m_strDefaultDirSort + strNameOld;
			CString strNewPath = ((CVeryIEApp*)AfxGetApp())->m_strDefaultDirSort + strName;
